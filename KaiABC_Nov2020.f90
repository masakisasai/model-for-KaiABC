...
module sim_parameters
  ...
  double precision,parameter :: dt=0.001     !A step width of dynamic
  
!  When the output data for FFT is needed, use the following  by deleting the symbol for commenting out

  !  integer,parameter :: ifft=1
  !  integer,parameter :: no=100000+100*2**15   !Max number of simulation steps
  
  integer,parameter :: ifft=0
  integer,parameter :: no=500000            !Max number of simulation steps
  ...
  integer,parameter :: iinput=0       ! iinput=0 not to read the initial data from the file
                                      ! iinput=1 to read the initial data from a file for continuation
                                      ! iinput =2 to read the initial data from two files to simulate mixing
  integer,parameter :: ioutput=0
                                      ! ioutput=0 for no output on the final data
                                      ! ioutput=1 to save the data at step=ioutstepa in a file
                                      ! ioutput=2 to save the data at step=ioutstepa and outstepb in each of two files
  integer,parameter :: ioutstepa=79000  
  integer,parameter :: ioutstepb=60000
  
  
  double precision,parameter :: cT=30.0 +273.15   !Temperature at the beginning
  double precision,parameter :: cT2=30.0 +273.15   !Temperature later than the itempshift step

!Structure modulation
  double precision,parameter :: tc0=2.0     !Shifting the average W
  double precision,parameter :: tc1=4.0     !Negative feedback strength to determine W
  double precision,parameter :: tc2=4.0     !Positive feedback strength to determine W
  double precision,parameter :: tc2kaiA=4.0     !Positive feedback strength to determine W  
...
!KaiAB abiudance  
  ...
  double precision,parameter :: Atot0=1.0*N           !Total number of KaiA monomers
  double precision,parameter :: Btot0=6.0*N           !Total number of KaiB monomers  
  double precision,parameter :: Atot=1.0*N           !Total number of KaiA monomers
...
!Phoshorylation/dephosphorylation 
  double precision,parameter :: trk1=0.3    !Rate of phosphorylation
  double precision,parameter :: tdk1=0.3    !Rate of dephosphorylation
...
!Binding/unbinding rates of KaiABC       
  ...
  double precision,parameter :: thB0=0.5/(Btot0)        !Rate coefficient of binding KaiB to KaiC
  double precision,parameter :: tfB0=1.0                !Rate of unbinding KaiB from KaiC
  double precision,parameter :: thBA=100.0/(Atot0)        !Rate coefficient of binding KaiA to KaiCB
...
!Parameters for ATP hydrolysis 
  double precision,parameter :: aw=1.0     !Modulation coefficient of duration on W    
  double precision,parameter :: aq=1.0     !Strength of perturbation due to the Pi release
!    double precision,parameter :: aq=0.0
  
!ATP hydrolysis rate(frequency) and duration (Notice that duration should be changed inversely!!!)
    double precision,parameter :: tomega=1.0  !Basic frequency of Pi release
    double precision,parameter :: tdelta0=1.0  !Duration steps of perturbation due to the Pi release
!  double precision,parameter :: tdelta0=2.0/tomega  !Duration steps of perturbation due to the Pi release
...
!============= File opening  =======================
      ...
      open(unit=30, file='u30.dat', status='replace')
...
!============= Loop of dynamics =======================
  ...
  T=cT/cT
  call Tmodulation(c0,c1,c2,c2kaiA,am,rk1,dk1,omega,delta0,hA0,fA0,hB0,fB0,hBA,fBA,T)
  call ATPhyd1(U,W,fw,q,qq,qq2,qt,Pha2,jstep,istep,iseed,ireact,c0,am,rk1,dk1,omega,delta0,hA0,fA0,hB0,fB0,hBA,fBA,T)
  ...
  call diffmodel(Pha2,PB,U,W,q,Utot,Wtot,c0,c1,c2,c2kaiA,am,rk1,dk1,omega,delta0, &
                       hA0,fA0,hB0,fB0,hBA,fBA,T,iseed)  
  time=0
            Uave=Utot/dble(N)
            ...
            ireact2=ireact
            qdiff=real(irdiff)/(6.0*N)
               write(49,*) time,-qt
               write(50,*) time,qdiff*10
...
!*****************************************************
subroutine Tmodulation(c0,c1,c2,c2kaiA,am,rk1,dk1,omega,delta0,hA0,fA0,hB0,fB0,hBA,fBA,T)
    use sim_parameters
    implicit none
    
    double precision ap,ap2,ap3,ap4,ap5,ap7,ap10,ap12,ap15,ap20,gp
    double precision c0,c1,c2,c2kaiA,am,rk1,dk1,omega,delta0,hA0,fA0,hB0,fB0,hBA,fBA,T
    ...
    ap=exp(1-1/T)
       ...
       gp=2.0            !shifting the average W
       !       c0=tc0*(0.8+gp*(1.0-T))   !Shifting the average W
       c0=tc0/ap5
       c1=tc1
       c2=tc2
       c2kaiA=tc2kaiA
       ...
       am=tam*ap5             !Amplitude of soft-spin potential
       
       rk1=trk1*ap5           !Rate of phosphorylation
       dk1=tdk1*ap5           !Rate of dephosphorylation
  
       omega=tomega           !Basic frequency of Pi release
       delta0=tdelta0/ap15     !Duration steps of perturbation due to the Pi release  
      
       hA0=thA0*ap5          !Rate coefficient of binding KaiA to KaiC
       fA0=tfA0*ap5          !Rate of unbinding KaiA from KaiC
       hB0=thB0*ap5          !Rate coefficient of binding KaiB to KaiC
       fB0=tfB0*ap5         !Rate of unbinding KaiB from KaiC
       hBA=thBA*ap15          !Rate coefficient of binding KaiA to KaiCB 
...
!************************************************************************
subroutine ATPhyd1(U,W,fw,q,qq,qq2,qt,Pha2,jstep,istep,iseed,ireact,c0,am,rk1,dk1,omega,delta0,hA0,fA0,hB0,fB0,hBA,fBA,T)
  
  use sim_parameters
  implicit none
  
  integer i,k,istep,iseed,iwait,ireact
  double precision s,r8_uniform_01,wq,r8_normal_01,twq
...
subroutine binding(sfA,sPha2,W,Amol,Bmol,Pha2,PB,q,istep,c0,c1,c2,c2kaiA,am,rk1,dk1, &
  ...
  g1=gkBA*Amol/(1+gkBA*Amol)
    ...
    do k=1,N
       ...
       Pha2(k)=gkA*Ph0(k)*Amol       
       pt=PBt(k)+Ph0(k)+Pha2(k)
...
subroutine diffmodel(Pha2,PB,U,W,q,Utot,Wtot,c0,c1,c2,c2kaiA,am,rk1,dk1,omega,delta0, &
  ...
  double precision c0,c1,c2,c2kaiA,am,rk1,dk1,omega,delta0,hA0,fA0,hB0,fB0,hBA,fBA,T,P6
...
!            dkk=(1-w(k))*0.5
         ...
         Do i=1,6
            sb=sb+PB(i,k)
         Enddo
         W(k) = tanh(( c0 -c1*U(k) +c2kaiA*Pha2(k) -c2*sb +q(k) )/T)
         Utot=Utot+U(k)
         Wtot=Wtot+W(k)
     enddo
